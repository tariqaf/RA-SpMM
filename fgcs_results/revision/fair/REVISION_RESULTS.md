...
All performance values in this report are generated from strictly correct rows with matching warm/warm or cold/cold lifecycle comparisons.
...
- Graphs measured: 51
- Mean full path, CPU-resident input: 2958.233425 ms
- Mean full path, GPU-resident input including required transfer: 2936.571739 ms
- The lightweight degree-moment kernel is reported separately and is not used as the production router cost.

## Profiling

See `profile/PROFILE_SUMMARY.md` for per-pair metrics and `profile/PROFILE_BOTTLENECKS.md` for grouped bottlenecks.
