...
# Final deployed roster: five kernel families / eight kernel/precision paths
# (each tile family has an FP16 and a TF32-direct path, router-selected by the
# degree/size gate).
# Deployed candidate set: RODE_ENHANCED retired — zero unique
# oracle wins and zero leave-one-out contribution on the evaluated suite
# oracle wins and zero leave-one-out contribution on the evaluated suite;
# retained in-tree as ablation evidence.
...
DEFAULT_RESULTS = "fgcs_results/revision/tf32/final_fair_v3.csv"


def route_with_rules(avg_nnz, degree_cv, M, N, nnz, *, disabled_rules=()):
    """
    Python mirror of the deployed seven-rule router.

    Seven rules, evaluated top-to-bottom; first match wins. Default
    ...
    def tile(kernel):
        ...
        # Precision dimension: the TF32 path deletes the B->FP16
    ...
    # 1. Extreme-skew sparse tails (com-youtube, cv ~ 9.7): the binned CSR
    #    kernel with flattened hub chunks dominates every tile variant.
    if 1 not in disabled and cv >= 5.0:
        return "ZERO_OVERHEAD_CSR"

    # 2. Uniform / near-uniform family (road networks, synthetic uniform,
    #    synthetic communities, uniform dense-small).
    if 2 not in disabled and cv < 0.7:
        if d < 4.5:
            # roadNet-*, uniform d=3: CSR wins at narrow N; the staged/TF32
            # tile path overtakes it at wide N.
    ...
    # 3. Very skewed (cv >= 3): Yelp-class large -> COMMUNITY_TC,
    #    Flickr-class medium -> balance-split SEGMENT_HYBRID.
    if 3 not in disabled and cv >= 3.0:
        return tile("COMMUNITY_TC") if M >= 250000 else tile("SEGMENT_HYBRID")

    # 4. Dense rows (Reddit, ogbn-proteins, gplus, high-d skewed synth):
    #    locality-ordered windows maximize vector reuse.
    if 4 not in disabled and d >= 40.0:
        return tile("COMMUNITY_TC")

    # 5. Small dense (amazon-photo/computers): cuSPARSE at narrow N,
    #    balance-split at mid N, locality windows at wide N.
    if 5 not in disabled and M < 20000 and d >= 25.0:
        if N < 96:
            return "CUSPARSE"
        if N < 256:
    ...
    # 6. Heavy mixed (synth_mixed_v2..v5: d >= 25, cv >= 1.8).
    if 6 not in disabled and d >= 25.0 and cv >= 1.8:
        return tile("COMMUNITY_TC")

    # 7. Web-locality sparse (web-Google, web-Stanford): large M, low d,
    #    moderate cv.
    if 7 not in disabled and d < 9.0 and 1.10 <= cv <= 1.45 and M >= 250000:
        # Web graphs: the locality sort raises gather reuse enough that the
        # TF32 path wins here even above the generic degree gate.
...
def simple_router(avg_nnz, degree_cv, M, N, nnz):
    """Return the deployed rule-tree decision with all seven rules enabled."""
...
def main():
    ...
    for (dataset, N), kernel_times in sorted(pairs.items()):
        ...
        # Canonical oracle (paper definition): cuSPARSE is a floor member of the
        # candidate set, so the oracle never trails cuSPARSE and no Router/Oracle
        # ratio exceeds 1.0.
        best_custom = min(kernel_times, key=kernel_times.get)
        best_custom_ms = kernel_times[best_custom]
        oracle_ms = min(best_custom_ms, cusparse_ms)                 # cuSPARSE floor
        oracle_kernel = "CUSPARSE" if cusparse_ms <= best_custom_ms else best_custom
        oracle_speed = cusparse_ms / oracle_ms

        # Router: what would our router pick? (Rule 5 may pick CUSPARSE.)
        ...
        ratio = router_speed / oracle_speed                         # deployed, floored
        ratios.append(ratio)
        ...
        is_hit = router_speed >= oracle_speed - 1e-9                 # deployed, floored
        is_hit_kernelonly = (router_kernel == best_custom)          # exact custom-path match
        hit = is_hit
        quality_rows.append({
            'dataset': dataset,
            'category': category,
            ...
            'oracle_custom_kernel': best_custom,
            'oracle_ms': oracle_ms,
            ...
            'is_hit': is_hit,
            'is_hit_kernelonly': is_hit_kernelonly,
        })
        if hit:
            hits += 1
        if is_hit_kernelonly:
            hits_kernelonly += 1
        else:
            misses.append((dataset, N, category, router_kernel, best_custom,
    ...
    print(f"{'Router hit rate (deployed)':<40s} {hits}/{total} ({100*hits/max(1,total):.1f}%)")
    print(f"{'Router hit rate (kernel-only)':<40s} {hits_kernelonly}/{total} ({100*hits_kernelonly/max(1,total):.1f}%)")
    ...
    if args.output:
        fieldnames = [
            ...
            'oracle_kernel', 'oracle_custom_kernel', 'oracle_ms', 'oracle_speedup',
            'router_kernel', 'router_ms', 'router_speedup', 'router_oracle_ratio',
            'is_hit', 'is_hit_kernelonly',
        ]
