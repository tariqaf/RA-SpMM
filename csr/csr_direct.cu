// ============================================================================
// csr_direct.cu - Native-CSR SpMM with an exact-fit subwarp strategy
//
// CSR_DIRECT audit summary:
// - Primary path (all evaluated feature widths): each output row is owned by a
//   W-lane subwarp with S float4 accumulators per lane, selected solely from N --
//   (W,S) = (16,1) at N=64, (32,1) at N=128, (32,2) at N=256, (32,4) at N=512.
//   A warp therefore covers 32/W rows and a 128-thread block covers 4*(32/W).
// - Sparse (column, value) pairs are broadcast across the subwarp with __shfl_sync;
//   the dense operand is read in float4 chunks.
// - Fallback (widths the vectorized path does not cover, or unaligned buffers):
//   graduated rows-per-warp by average degree -- 8 rows if d<=2.5, 4 if d<=6,
//   2 if d<=12, else one row per warp. Not exercised at the evaluated widths.
