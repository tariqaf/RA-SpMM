#!/usr/bin/env python3
"""
GraphSAGE training-step benchmark for the deployed router (five kernel families /
eight kernel-precision paths), cuSPARSE, a fixed TC_DIRECT path, and the PyG
torch_sparse SparseTensor backend. The aggregation operator is the row-normalized
mean A_mean = D^-1 A (built backend-neutrally); the self representation is
concatenated separately, so no self-loops are added to the aggregation matrix.
...
Two SpMM calls occur per forward pass, at `(M, in_dim)` and
`(M, hidden_dim)`.
"""
import argparse
import csv
...
class SAGEBench(nn.Module):
    ...
    def forward(self, X: torch.Tensor, graph: GraphBackend, backend: str) -> torch.Tensor:
        # Layer 1: mean aggregator. The adjacency is row-normalized to D^-1 A
        # before any backend is built, so this SpMM is a true mean; the router's
        # decision depends on
...
def benchmark_dataset(
    ...
    # Faithful GraphSAGE mean operator A_mean = D^-1 A (no self-loop; the model concatenates the self branch).
    from gnn_bench.router_vs_baselines_gcn import build_aggregation_operator, operator_descriptors
    csr = build_aggregation_operator(csr, "graphsage")
