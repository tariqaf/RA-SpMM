#!/usr/bin/env python3
"""
GCN end-to-end benchmark for the deployed router (five kernel families / eight
kernel-precision paths), cuSPARSE, a fixed TC_DIRECT path, and the PyG
torch_sparse SparseTensor backend. The GCN operator is the faithful normalized
adjacency A_hat = D~^-1/2 (A + I) D~^-1/2 (built backend-neutrally).
...
DATASETS: Dict[str, DatasetSpec] = {
    ...
    "Yelp":             DatasetSpec("Yelp",             "Yelp.npz",             in_dim=300, out_dim=100, hidden_dim=128),
...
# The six mixed-precision tile paths that receive the relaxed correctness
# tolerance (FP32 accumulation; the relaxation covers input conversion and
# summation order, not accumulation precision).
TC_KERNELS = {"TC_DIRECT", "TC_DIRECT_TF32", "COMMUNITY_TC", "COMMUNITY_TC_TF32",
              "SEGMENT_HYBRID", "SEGMENT_HYBRID_TF32"}
_TF32_FAMILIES = {"TC_DIRECT", "COMMUNITY_TC", "SEGMENT_HYBRID"}


def resolve_router_label(plan: Dict[str, object]) -> str:
    """Resolve the executed kernel label from the router plan: the router
    returns the kernel family in chosen_path and the precision separately in
    use_tf32, so a tile family with use_tf32 runs its *_TF32 path."""
    family = str(plan["chosen_path"])
    use_tf32 = bool(plan.get("use_tf32", False))
    return f"{family}_TF32" if (use_tf32 and family in _TF32_FAMILIES) else family


def build_aggregation_operator(csr: "sp.csr_matrix", model: str) -> "sp.csr_matrix":
    """Faithful per-model aggregation matrix, built ONCE from the raw adjacency
    before any backend/reference is constructed so the router, cuSPARSE, PyG,
    and the correctness reference all consume the identical matrix.

      GCN  : A_hat = D~^-1/2 (A + I) D~^-1/2, D~ = diag(rowsum(A + I));
             the diagonal is added ONLY where absent (no double self-loop).
      SAGE : A_mean = D^-1 A (row-mean); zero-degree rows stay zero; no self-loop
             (the model concatenates the self branch separately).
      GIN  : raw A unchanged ((1 + eps) h supplies the self term in the model).

    Descriptors therefore differ per model (GCN's self-loops raise nnz by up to
    M and shift degrees); the caller records them and does not stabilize them.
    """
    ...
    if m == "gin":
        out = A.copy()
    elif m in ("graphsage", "sage"):
        deg = np.asarray(A.sum(axis=1)).ravel()
        dinv = np.divide(1.0, deg, out=np.zeros_like(deg), where=deg > 0)
        out = sp.diags(dinv) @ A                      # D^-1 A ; zero-deg rows stay zero
    elif m == "gcn":
        diag = A.diagonal()
        eye = sp.eye(A.shape[0], format="csr", dtype=np.float64)
        eye.data[diag != 0] = 0.0                     # do not double-add existing self-loops
        Ahat = (A + eye).tocsr()
        Ahat.eliminate_zeros()
        d = np.asarray(Ahat.sum(axis=1)).ravel()
        dis = np.divide(1.0, np.sqrt(d), out=np.zeros_like(d), where=d > 0)
        D = sp.diags(dis)
        out = D @ Ahat @ D                            # D~^-1/2 (A + I) D~^-1/2
    else:
        raise ValueError(f"unknown model for aggregation operator: {model}")
    out = out.tocsr().astype(np.float32)
    out.sort_indices()
    return out


def operator_descriptors(csr: "sp.csr_matrix") -> Dict[str, float]:
    """M, nnz, d_bar, CV_d of an aggregation matrix (population CV of row nnz)."""
    M = int(csr.shape[0]); nnz = int(csr.nnz)
    deg = np.diff(csr.indptr).astype(np.float64)
    mean = deg.mean() if M else 0.0
    cv = float(deg.std() / mean) if mean > 0 else 0.0
    return {"M": M, "nnz": nnz, "d_bar": round(nnz / max(1, M), 5), "CV_d": round(cv, 5)}
...
class GraphBackend:
    ...
    def _get_executor(self, backend: str, transpose: bool, ncols: int):
        ...
        # -- Router: ask the C++ router which path to use --
            ...
            # The router returns the kernel FAMILY in chosen_path and the
            # precision separately in use_tf32; resolve the executed label so
            # the TF32 paths actually run (the old code read chosen_path only
            # and therefore always ran FP16 on the tile kernels).
            path = resolve_router_label(plan)
            ...
            # Paper portfolio paths. The FP16 and TF32 variants of a tile
            # kernel share one precision-agnostic ME-BCRS plan (same make_*_plan);
            # only the run function differs, so the resolved label is stored as
            # the executor label to drive the correct run branch.
            elif path in ("TC_DIRECT", "TC_DIRECT_TF32"):
                wrapper = spmm_next.make_tc_direct_plan(
                    tensors["rowptr_cpu"], tensors["colind_cpu"], tensors["vals_cpu"],
                    M, K, int(ncols),
                )
                executor = (path, wrapper)
            elif path in ("COMMUNITY_TC", "COMMUNITY_TC_TF32"):
                ...
                executor = (path, wrapper)
            elif path in ("SEGMENT_HYBRID", "SEGMENT_HYBRID_TF32"):
    ...
    def run(self, B: torch.Tensor, backend: str, transpose: bool = False) -> torch.Tensor:
        ...
        if path == "COMMUNITY_TC_TF32":
            return spmm_next.run_community_tc_plan_tf32(wrapper, B)
        ...
        if path == "SEGMENT_HYBRID_TF32":
            return spmm_next.run_segment_hybrid_plan_tf32(
                wrapper, tensors["colind"], tensors["vals"], B,
            )
...
def benchmark_dataset(
    ...
    # Faithful GCN operator A_hat = D~^-1/2 (A + I) D~^-1/2, built once so every
    # backend and the correctness reference multiply the identical matrix.
    csr = build_aggregation_operator(csr, "gcn")
    op_desc = operator_descriptors(csr)
    ...
    for backend in backends:
        ...
        if backend == "router":
            for tag, ncols in (
                ...
                router_paths[tag] = resolve_router_label(plan)
