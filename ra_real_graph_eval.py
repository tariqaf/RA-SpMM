...
Loads graphs from paper_datasets.json and tests the deployed portfolio (five
kernel families / eight kernel-precision paths) against cuSPARSE.
...
# factor for the mixed-precision mma tile paths (mma.m16n8k8; FP32 accumulate).
BASE_ATOL = 1e-3
TC_EXTRA_FACTOR = 10.0  # mixed-precision tolerance: covers FP16/TF32 input
                        # conversion and summation order (accumulation is FP32)
...
# Deployed reproduction roster: the eight paths the router actually selects
# (five kernel families; the three tile kernels each have an FP16 and a
# TF32-direct precision path). RODE_ENHANCED is retired from deployment (zero
# unique oracle wins, zero leave-one-out contribution) and lives only in the
# paper-audit ablation roster below.
DEPLOYED_KERNELS = [
    "CSR_DIRECT",
    "ZERO_OVERHEAD_CSR",
    "TC_DIRECT",
...
# paper-audit = deployed roster + the retired RODE_ENHANCED ablation candidate.
PAPER_AUDIT_KERNELS = DEPLOYED_KERNELS + ["RODE_ENHANCED"]

PORTFOLIOS = {"deployed": DEPLOYED_KERNELS, "paper-audit": PAPER_AUDIT_KERNELS}

# Backwards-compatible aliases (the default roster is now the deployed eight).
ALL_KERNELS = DEPLOYED_KERNELS
EXPERIMENTAL_KERNELS = ["RODE_ENHANCED", "TC_DIRECT_ZC", "TC_DIRECT_ZC_TF32"]
...
def main():
    ...
    parser.add_argument("--portfolio", choices=sorted(PORTFOLIOS), default="deployed",
                        help="Reproduction roster: 'deployed' = the eight deployed "
                             "kernel/precision paths; 'paper-audit' = deployed + RODE_ENHANCED")
    parser.add_argument("--kernels", default=None,
                        help="Comma-separated custom kernels (overrides --portfolio); "
                             "cuSPARSE is always measured")
    ...
    roster = PORTFOLIOS[args.portfolio]
    selected_kernels = ([name.strip() for name in args.kernels.split(",") if name.strip()]
                        if args.kernels else list(roster))
    invalid_kernels = sorted(
        set(selected_kernels) - set(DEPLOYED_KERNELS) - set(EXPERIMENTAL_KERNELS))
    if invalid_kernels:
