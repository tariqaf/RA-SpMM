...
// The TC kernels are numerically correct; earlier discrepancies were caused by
// double-symmetrized road graphs (duplicate edges), not FP16 precision issues.
    ...
    int M, int K, int N)
{
    ...
    // Earlier discrepancies were caused by double-symmetrized road graphs
    // (duplicate edges inflating the reference), not TC kernel errors.
