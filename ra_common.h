...
//
// Deployed portfolio: five kernel families / eight kernel-precision paths.
// Two run on native CSR with no format construction:
//   - CSR_DIRECT          (csr/csr_direct.cu)        subwarp-per-row, exact-fit float4
//   - ZERO_OVERHEAD_CSR   (csr/ra_zero_overhead.cu)  degree-binned dispatch
// Three share a blocked Tensor-Core engine, each with an FP16 and a TF32-direct path
// selected by the router's precision rule:
//   - TC_DIRECT           (tc/ra_tc_direct.cu)       blocked 8x1 mma.m16n8k8, natural
//                                                    row order
//   - COMMUNITY_TC        (tc/ra_community_tc.cu)    deterministic locality-ordering sort
//                                                    by leading neighbour column, then TC
//   - SEGMENT_HYBRID      (tc/ra_segment_hybrid.cu)  balance-split windows with a
//                                                    deterministic scratch-slot merge
//                                                    ("hybrid" = direct vs split-window
//                                                    execution, not TC vs CUDA core)
// Plus: CUSPARSE (vendor guardrail, dispatched when no specialist improves on it)
//
// Ablation-only kernels (kept in tree for reproducibility, not deployed):
//   - csr/ra_rode_enhanced.cu (retired: no unique oracle value on the evaluated suite)
...
//     tc/tc_sparse.cu
//
// Targets: NVIDIA Ampere (sm_86; RTX 3090) and Ada Lovelace (sm_89; RTX 4090),
// CUDA 11.8/12.x, cuSPARSE 11.x/12.x
// ============================================================================
#pragma once
...
struct RARodeEnhancedPlan {
    ...
    // Flattened long-row chunk descriptors: the pipelined
