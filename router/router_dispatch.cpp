...
//
// Deployed portfolio (paper-facing): five kernel families, eight kernel/precision
// paths (each tile family has an FP16 and a TF32-direct path):
// - CSR_DIRECT
// - ZERO_OVERHEAD_CSR
...
// RODE_ENHANCED is retired from deployment (ablation/paper-audit roster only).
//
    ...
    Portfolio portfolio)
{
    ...
    // =================================================================
    // Deployed kernel routing: five families / eight kernel-precision paths
    //
    ...
    if (path_in_portfolio(NextPath::TC_DIRECT, portfolio)) {
        ...
        // ============================================================
        // Production seven-rule router. Mirrors
        ...
        auto pick_tile = [&](NextPath p, const char* reason,
            ...
            if (tile_ok && feas(p)) {
                ...
                extended_risk = 0.06f;
                // Precision dimension: TF32 wins where the deleted
        ...
        if (cv >= 5.0f) {
            // Rule 1: extreme-skew sparse tails (com-youtube).
            extended_choice = NextPath::ZERO_OVERHEAD_CSR;
            extended_reason = "zero_overhead_extreme_skew";
            extended_risk = 0.06f;
        } else if (cv < 0.7f) {
            // Rule 2: uniform / near-uniform family.
            if (d < 4.5f) {
                if (N <= 256) {
                    extended_choice = NextPath::CSR_DIRECT;
            ...
            // Rule 3: very skewed (Yelp-class large vs Flickr-class medium).
            if (M >= 250000) {
            ...
            // Rule 4: dense rows (Reddit, ogbn-proteins, gplus, dense skewed).
            pick_tile(NextPath::COMMUNITY_TC, "community_tc_dense_rows");
        } else if (M < 20000 && d >= 25.0f) {
            // Rule 5: small dense (amazon-photo/computers).
            if (N < 96) {
